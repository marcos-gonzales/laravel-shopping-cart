<template>
    <div class="mx-auto">
        ...
        <div class="grid grid-cols-3 gap-6 ">
            <div v-for="product in orders" class="gap-x-8 border border-sky-100 py-6 px-2 bg-slate-700 my-12 flex flex-col cursor-pointer" @click="summaryShow(product)">
                <div class="flex justify-between">
                    <span class="text-center text-lg text-yellow-200 font-bold contrast-125"> {{moment(product.created_at)}} </span>
                    <span class="text-center text-lg">Order Total: <i class="fa-solid fa-sack-dollar text-yellow-600"></i> {{product.total}}</span>
                </div>
...
<!--                        <img :src="prod.file_path" alt="{{prod.name}}" style="height: 100px; width: 100px; word-break: keep-all" class="col-span-1 border-2 border-gray-600">-->
                         <img :src="'http://localhost:8000/storage/products/' + prod.id + '/' +  prod.file_path" :alt="prod.name" style="height: 100px; width: 100px; word-break: keep-all" class="col-span-1 border-2 border-gray-600">
